...
/**
 * Which leg of the trip a subtree describes.
 *
 * SAS's date picker splits its answer into `outbound` and `inbound` maps, and
 * neither carries a route — the airports are only in the request. Without this,
 * every price took the request's origin and destination, so the return half was
 * stored as though it flew the outbound way round, and dedupe then quietly
 * discarded returns that happened to cost the same as their outbound.
 *
 * `associatedFares` flips the leg rather than naming it: inside an outbound
 * date, those entries are the return dates it can pair with, priced as the
 * inbound leg. That is visible in the payload — outbound entries carry one
 * tax figure and their associatedFares carry the other, exactly swapped under
 * `inbound`.
 */
const OUTBOUND_KEY = /^(outbound|outward|departure|departing|depart|out|going)$/i;
const INBOUND_KEY = /^(inbound|return|returning|homebound|home|back)$/i;
const PAIRED_KEY = /^(associated|paired|combined|matching)/i;
  ...
  function walk(node, ctx, path, depth) {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      found.push({
        ...
        direction: nextCtx.direction ?? null,
    ...
    for (const [key, value] of Object.entries(node)) {
      // A key can BE the date. SAS's date picker returns
      // {"outbound": {"2026-09-08": {points: 10000}}} — a date-keyed map rather
      // than a {date: "..."} field — so reading dates only from values left
      // every price undated, and `pull` drops undated prices. The nearest
      // enclosing date key wins: in {"2026-09-08": {associatedFares:
      // {"2026-09-19": ...}}} the inner entry really is the 19th's fare.
      const keyDate = firstDate(key);
      let childCtx = keyDate ? { ...nextCtx, date: keyDate } : nextCtx;

      // The container names the leg, and a paired-fares container inverts it.
      if (OUTBOUND_KEY.test(key)) childCtx = { ...childCtx, direction: "outbound" };
      else if (INBOUND_KEY.test(key)) childCtx = { ...childCtx, direction: "inbound" };
      else if (PAIRED_KEY.test(key) && childCtx.direction) {
        childCtx = { ...childCtx, direction: childCtx.direction === "outbound" ? "inbound" : "outbound" };
      }

      walk(value, childCtx, path ? `${path}.${key}` : key, depth + 1);
    }
  }
...
function dedupe(offers) {
  ...
  for (const o of offers) {
    // Direction is part of identity: an outbound and a return on the same date
    // for the same points are two different flights, not one seen twice.
    const key = [o.origin, o.destination, o.date, (o.cabin || "").toLowerCase(),
                 o.points, o.flight, o.direction || ""].join("|");
