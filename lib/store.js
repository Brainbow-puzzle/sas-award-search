...
class Store {
  ...
  upsertOffers(rows, { now = new Date().toISOString() } = {}) {
    const insertOffer = this.db.prepare(`
      INSERT INTO offers (origin, destination, depart_date, cabin, flight, points,
                          cash, currency, seats, source, first_seen, last_seen)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    const insertObs = this.db.prepare(`
      INSERT INTO observations (origin, destination, depart_date, cabin, flight,
                                points, cash, currency, seats, seen_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    try {
      for (const r of rows) {
        ...
        insertOffer.run(r.origin, r.destination, r.date, cabin, flight, r.points,
          r.cash ?? null, r.currency ?? null, r.seats ?? null, r.source ?? null, now, now);
        insertObs.run(r.origin, r.destination, r.date, cabin, flight, r.points,
          r.cash ?? null, r.currency ?? null, r.seats ?? null, now);
        stored++;
      }
      this.db.exec("COMMIT");
    } catch (e) {
  ...
  query({
    ...
    saverOnly = false, order = "points", limit = 50,
  } = {}) {
    const where = [];
    const params = [];
  ...
  cheapestPerDestination({ origin = null, cabin = null, from = null, to = null, limit = 100 } = {}) {
    const where = [];
    const params = [];
