...
async function launch({ headless = false, storageState = null, slowMo = 0, channel = null } = {}) {
  ...
  try {
    ...
    // Asking for a browser this machine does not have should degrade, not stop.
    if (!channel) throw e;
    console.log(`\nNo "${channel}" installed — falling back to the bundled Chromium.`);
    console.log("SAS refuses that build, so if requests come back 403, install Chrome.\n");
