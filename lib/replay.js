...
function buildRecipe(request, observed, meta = {}) {
  return {
    ...
    // How many distinct dates the captured response carried. Evidence that
    // outranks the URL's date format when deciding how finely to iterate.
    datesInCapture: meta.datesInCapture ?? null,
 ...
 */
/** A response covering at least this many dates was answering for a window. */
const WINDOW_DATE_COUNT = 7;
...
function dateGranularity(recipe) {
  ...
  const namesADay = specs.some((sp) => DAY_SPECS.has(sp));
  if (!namesADay) return "month";

  // Naming a full date does not mean returning only that date. SAS's date
  // picker is addressed as departureDate=2026-09-01 yet answers with the whole
  // of September, so believing the URL would fire ~30 identical requests per
  // month. What the capture actually returned settles it.
  if ((recipe.datesInCapture || 0) >= WINDOW_DATE_COUNT) return "month";
  return "day";
}
...
/**
 * Replay from inside a loaded page, rather than beside it.
 *
 * `replay` above uses Playwright's request context: it shares the browser's
 * cookie jar but is a separate HTTP client, and www.sas.dk answers it 403. The
 * site's own scripts call this endpoint as a same-origin fetch from a page that
 * is already open, and Cloudflare issues its __cf_bm cookie in response to that
 * traffic rather than to a homepage visit.
 *
 * So issue the request the way the page does. The browser supplies Origin,
 * Referer, sec-fetch-* and cookies itself — they are forbidden headers that
 * script cannot set, which is precisely why setting them by hand never worked.
 *
 * The page must already be on the target origin, or the fetch is cross-origin
 * and the browser blocks it before it leaves.
 */
async function replayInPage(page, recipe, vars, { timeout = 30000 } = {}) {
  const url = materialize(recipe.urlTemplate, vars);
  const body = recipe.bodyTemplate ? materialize(recipe.bodyTemplate, vars) : null;
  ...
  const result = await page.evaluate(
    async ({ url, method, body, sendsBody, timeout }) => {
    ...
    { url, method, body, sendsBody, timeout },
  );

  let json = null;
  try { json = JSON.parse(result.text); } catch { /* not JSON: blocked, or an error page */ }
  return { status: result.status, json, url, bodyLength: (result.text || "").length, error: result.error || null };
}
...
module.exports = {
  parameterize, materialize, buildRecipe, replay, replayInPage, needsDate, dateGranularity,
  selectRecipe,
